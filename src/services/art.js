...
const getImages = async (session, prompt) => {
  ...
  const url = `${BING_URL}/images/create?q=${encodeURIComponent(prompt)}&rt=3&FORM=GENCRE`; // force use rt=3
  console.log(url);
...
export const generateImagesLinks = async (prompt) => {
  const authCookie = import.meta.env.VITE_BING_IMAGE_COOKIE;
  const outputDir = `${import.meta.env.TEMP_DIR || "/tmp"}/${prompt}`;
